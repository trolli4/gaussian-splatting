...
BASE_DATASET_PATH="/home/s76mfroe_hpc/own-scenes"

# Log directory
LOG_DIR="./logs"
mkdir -p "$LOG_DIR"

scenes="doll person roundabout castle"

# Loop over all folders in dataset path
for folder in $scenes; do
    if [ -d "$BASE_DATASET_PATH"/"$folder" ]; then
        folder_name=$(basename "$folder")
        log_file="${LOG_DIR}/original/eval_4/${folder_name}.out"
        model_path="output/original/eval_4/${folder_name}"
